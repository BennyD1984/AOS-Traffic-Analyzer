    ...
    for round in range(1, arg_repetitions):
        ...
        for port in range(len(all_active_ports)):
            ...
            if re.search(rxp1_in_bits, output_clean):
                print(rxp1_in_bits)
                print(output_clean)
                y = re.match(rxp1_in_bits, output_clean)
                print(y)
                int_bit_s = int(re.search(rxp1_in_bits, output_clean).group(2))
                print("IN - Before IF check: " + str(int_bit_s))
                if int_bit_s > 0:
                    print("IN - Yes outbits greater 0\n")
                    print("IN - int_bits_s before division: " + str(int_bit_s) + "\n")
                    int_bit_s = int_bit_s / 1024 / 1024  # convert to Megabit/s and check Device/0 Error
                    print("IN - int_bits_s after division: " + str(int_bit_s) + "\n")
                else:
                    int_bit_s = 0.00
                value_in_bits = "{:0.2f}".format(int_bit_s)  # Format settings to two digits after commata

            if re.search(rxp1_out_bits, output_clean):
                x = re.match(rxp1_out_bits, output_clean)
                print(x)
                int_bit_s = int(re.search(rxp1_out_bits, output_clean).group(2))
                print("OUT - Before IF check: " + str(int_bit_s))
                if int_bit_s > 0:
                    print("OUT - Yes outbits greater 0\n")
                    print("OUT - int_bits_s before division: " + str(int_bit_s) + "\n")
                    int_bit_s = int_bit_s / 1024 / 1024  # convert to Megabits/s and check Device/0 Error
                    print("OUT - int_bits_s after division: " + str(int_bit_s) + "\n")
                else:
                    int_bit_s = 0.00
